...
/**
 * Calculate subcontractor invoice status based on payments
 * @param invoiceId - The subcontractor invoice ID
 * @returns Promise<string> - Status: 'paid', 'partially_paid', or 'unpaid'
 */
export async function calculateSubcontractorInvoiceStatus(invoiceId: number): Promise<string> {
  // Get invoice with payment information
  const invoice = await prisma.projectSubcontractorInvoice.findUnique({
    where: { id: invoiceId },
    include: {
      paymentInvoices: {
        include: {
...
/**
 * Update subcontractor invoice status in the database
 * @param invoiceId - The subcontractor invoice ID
 * @returns Promise<void>
 */
export async function updateSubcontractorInvoiceStatus(invoiceId: number): Promise<void> {
  const status = await calculateSubcontractorInvoiceStatus(invoiceId);
  
  await prisma.projectSubcontractorInvoice.update({
    where: { id: invoiceId },
    data: { status },
  });
}
...
/**
 * Update status for multiple subcontractor invoices
 * @param invoiceIds - Array of subcontractor invoice IDs
 * @returns Promise<void>
 */
export async function updateMultipleSubcontractorInvoiceStatuses(invoiceIds: number[]): Promise<void> {
  await Promise.all(
    invoiceIds.map(invoiceId => updateSubcontractorInvoiceStatus(invoiceId))
  );
}
