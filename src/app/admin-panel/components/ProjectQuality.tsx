...
export default function ProjectQuality({ projectId, projectName, projectStartDate, projectEndDate }: ProjectQualityProps) {
  ...
  const latestRowsRef = useRef<{ e1Rows: E1LogRow[]; e2Rows: E1LogRow[] }>({
    e1Rows: e1LogRows,
    e2Rows: e2LogRows,
  });
  ...
  const fetchQualityData = useCallback(async () => {
    ...
    try {
      ...
      setE1LogRows(e1Rows);
      setE2LogRows(e2Rows);
      latestRowsRef.current = { e1Rows, e2Rows };
      setLastSavedAt(null);
      setSaveError(null);
    } catch (error: any) {
  ...
  const saveQualityData = useCallback(
    async (rowsToSave?: { e1Rows: E1LogRow[]; e2Rows: E1LogRow[] }) => {
      const payload = rowsToSave ?? latestRowsRef.current;
      if (!payload) return;

      const { e1Rows, e2Rows } = payload;
      if (e1Rows.length === 0 && e2Rows.length === 0) return;
      ...
      try {
        ...
        latestRowsRef.current = { e1Rows: nextE1Rows, e2Rows: nextE2Rows };
  ...
  const updateRowsAtIndex = (
    rows: E1LogRow[],
    rowIndex: number,
    patch: Partial<E1LogRow>
  ...
  const handleAddE1Row = () => {
    ...
    latestRowsRef.current = { e1Rows: updatedRows, e2Rows: e2LogRows };
  };
  ...
  const handleAddE2Row = () => {
    ...
    latestRowsRef.current = { e1Rows: e1LogRows, e2Rows: updatedRows };
  };
  ...
  const handleDeleteE1Row = (rowIndex: number) => {
    ...
    const nextState = { e1Rows: updatedE1Rows, e2Rows: e2LogRows };
  ...
  const handleDeleteE2Row = (rowIndex: number) => {
    ...
    const nextState = { e1Rows: e1LogRows, e2Rows: updatedE2Rows };
    latestRowsRef.current = nextState;
  ...
  }, [e1LogRows, e2LogRows, isLoading, saveQualityData]);
