...
export async function PUT(
  ...
  try {
    ...
    if (isNaN(projectSubcontractorId) || isNaN(paymentIdNum)) {
      return NextResponse.json(
        { success: false, error: 'Invalid project subcontractor or payment ID' },
        { status: 400 }
      );
    }
    ...
    // Verify the payment exists and belongs to this subcontractor
    const existingPayment = await prisma.projectSubcontractorPayment.findFirst({
      where: {
        id: paymentIdNum,
    ...
    if (!existingPayment) {
      return NextResponse.json(
        { success: false, error: 'Payment not found or does not belong to this subcontractor' },
        { status: 404 }
      );
    }
    ...
    await prisma.projectSubcontractorPaymentInvoice.deleteMany({
      where: { paymentId: paymentIdNum },
    });

    // Update payment
    const payment = await prisma.projectSubcontractorPayment.update({
      ...
      data: {
        ...
        paymentType: paymentMethod === 'Post Dated' ? paymentType : null,
         paymentDate: parseDateFromInput(paymentDate) || undefined,
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.projectSubcontractorPayment.delete({
      where: { id: paymentIdNum },
    });
