...
// Schema for validating imported staff data
const importStaffSchema = z.object({
  'Staff Name': z.string().min(1, 'Staff name is required'),
...
// POST - Import staff data from Excel file
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate headers
    const requiredHeaders = ['Staff Name'];
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      try {
        // Validate row data
        const validatedData = importStaffSchema.parse(rowData);
        
        // Convert to database format
        const staffData = {
          staffName: validatedData['Staff Name'],
        ...
        console.log('Validated staff data:', staffData);
    ...
    for (const staffData of processedData) {
      try {
        console.log('Processing staff:', staffData.staffName);
        
        // Check if staff already exists (by name or employee number)
        const existingStaff = await prisma.companyStaff.findFirst({
          where: {
            OR: [
              { staffName: staffData.staffName },
              ...(staffData.employeeNumber ? [{ employeeNumber: staffData.employeeNumber }] : [])
            ]
          }
        });

        if (existingStaff) {
          console.log('Updating existing staff:', existingStaff.id);
          ...
          console.log('Creating new staff:', staffData.staffName);
          // Create new staff
        ...
        console.error('Database error for staff:', staffData.staffName, error);
        results.errors.push({
          staffName: staffData.staffName,
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to import staff data' },
      { status: 500 }
    );
  }
}
