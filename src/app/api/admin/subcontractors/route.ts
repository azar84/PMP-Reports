...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use the same suppliers/vendors list - subcontractors come from the same vendor pool
    ...
    return NextResponse.json({
      success: true,
      data: {
        subcontractors: suppliers, // Return suppliers as subcontractors (same vendor pool)
        typeOfWorks,
      },
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use the same suppliers/vendors table - subcontractors are managed through the same vendor system
    const typeOfWorks = await resolveTypeOfWorks(body?.typeOfWorks);

    const subcontractor = await prisma.supplier.create({
      data: {
        name,
